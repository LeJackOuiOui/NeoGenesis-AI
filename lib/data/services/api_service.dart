...
class ApiService {
	...
	static bool isValidGmailEmail(String? value) {
		...
		return RegExp(r'^[a-z0-9._%+\-]+@(gmail|googlemail)\.com$').hasMatch(normalized);
	}
	...
	Future<UserModel> upsertEmployeeByEmail({
		...
		try {
			current = await client
					.from('profiles')
					.select('id, nombre, email, cargo, role, estado, created_at, salario_base, tipo_contrato, fecha_ingreso, baja_logica')
					.eq('email', normalizedEmail)
					.maybeSingle();
		} on PostgrestException catch (error) {
			if (!_isMissingColumnError(error)) {
		...
		if (current == null) {
			if (!isValidGmailEmail(normalizedEmail)) {
				throw Exception('El correo debe ser un Gmail válido para registrar un nuevo empleado.');
			}
			try {
